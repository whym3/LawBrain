										...
										{#if $_user && (showToolsButton || (toggleFilters && toggleFilters.length > 0) || showWebSearchButton || showImageGenerationButton || showCodeInterpreterButton)}
											...
											<div class="flex gap-1 items-center overflow-x-auto scrollbar-none flex-1">
												...
												{#if showCodeInterpreterButton}
													<Tooltip content={$i18n.t('Draft legal documents')} placement="top">
														<button
															aria-label={$i18n.t('Draft Documents')}
															on:click|preventDefault={() => {
																window.open('http://localhost:3000', '_blank');
															}}
															type="button"
															class="px-2 @xl:px-2.5 py-2 flex gap-1.5 items-center text-sm transition-colors duration-300 max-w-full overflow-hidden hover:bg-gray-50 dark:hover:bg-gray-800 bg-transparent text-gray-600 dark:text-gray-300 {($settings?.highContrastMode ??
															false)
																? 'm-1'
